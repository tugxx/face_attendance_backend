...
def get_incremental_sync_queryset(last_sync_str: str = None):
    ...
    users_query = UserModel.objects.select_related("profile", "biometric").only(
        ...
        "biometric__rfid_card_id",
        "biometric__face_embeddings",
        "biometric__is_active",
        "biometric__updated_at",
    )
    ...
    if last_sync_str:
        try:
            ...
            raise ValueError("Invalid last_sync datetime format.") from error
